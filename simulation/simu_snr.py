...
class Simulator(object):
                        ...
                        for snr in self.snr_list:
                            for i in range(num_rep):
                                ...
                                for model in model_list:
                                    ...
                                    if str(model) == 'Oracle':
                                        w_est = np.zeros(p)
                                        model_tmp = TailScreening()
                                        # w_est[supp_true] = model_tmp.grad_method(
                                        #     X=X[:, supp_true], 
                                        #     y=y, 
                                        #     w_init=np.ones(s) / s,
                                        #     prec=True
    ...
    def plot(self):
        ...
        markers = ['o', '>', '^', 's', 'D']
        palette = ['silver', 'deeppink', 'darkblue', 'purple', 'forestgreen']
        sns.lineplot(data=self.df, hue='model', x='n', y='exact', 
        ...
        sns.lineplot(data=self.df, hue='model', x='n', y='err', 
                    estimator='mean', errorbar=None, 
                    style='model',
                    markers=markers,
                    dashes=False,
        ...
        sns.lineplot(data=self.df, hue='model', x='n', y='time', 
                    estimator='mean', errorbar=None, 
                    style='model',
                    markers=markers,
                    dashes=False,
...
if __name__ == '__main__':
    ...
    df.to_csv('./results/snr/snr.csv', index=False)

    # conda activate skscope
    # cd C:\Users\uest\Desktop\new-self-regularization
    # python simu_snr.py > simu_snr_log.txt
    # start /min python simu_snr.py > simu_snr_log.txt 2>&1 &
    # nohup python -u simulation.py > simu_snr_log.txt 2>&1 &
    # jobs -l
